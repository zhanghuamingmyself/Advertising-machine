...
public class NearAdapter extends BaseAdapter {
    private List<Near> mData;//定义数据。
    private LayoutInflater mInflater;//定义Inflater,加载我们自定义的布局。
    private OpenAdapter.ViewHolder viewHolder;
    /*
    定义构造器，在Activity创建对象Adapter的时候将数据data和Inflater传入自定义的Adapter中进行处理。
    */
    public NearAdapter(LayoutInflater inflater, List<Near> data){
        mData=new ArrayList<Near>(100);
        mInflater = inflater;
        mData = data;
    }
    @Override
    ...
    public View getView(int position, View convertview, ViewGroup viewGroup) {
        ...
        if(convertview == null)
        {
            ...
            viewHolder = new OpenAdapter.ViewHolder();
            ...
            viewHolder = (OpenAdapter.ViewHolder)convertview.getTag();
        }

//获得对象
        Near item = mData.get(position);
